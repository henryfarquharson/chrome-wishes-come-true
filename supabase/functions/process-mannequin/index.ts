...
async function callAI(apiKey: string, messages: any[]) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 402) throw { status: 402, message: "Payment required. Please add credits to your Lovable workspace." };
  ...
  // Check for image in the Lovable AI gateway response format
  let imageUrl: string | undefined;

  // Format: choices[].message.images[].image_url.url
  imageUrl = data.choices?.[0]?.message?.images?.[0]?.image_url?.url;

  // Fallback: check inline_data in content array
  if (!imageUrl) {
  ...
  // Fallback: extract base64 from text content
  if (!imageUrl) {
    const textContent = typeof data.choices?.[0]?.message?.content === "string" ? data.choices[0].message.content : "";
  ...
  if (!imageUrl) {
    ...
    console.error("No image in response. Content:", preview);
